    ...
    <div className="min-h-screen">
      <div className="px-6 pt-6 pb-3">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Privát feladataim</h1>
            <p className="text-gray-500 text-sm">Csak te látod. Senki más — adminisztrátor sem.</p>
          </div>
        </div>
      </div>

      {error && (
        <div className="mx-6 mb-3 px-3 py-2 bg-red-50 border border-red-100 rounded-lg text-sm text-red-600 flex items-center justify-between">
          {error}
          <button onClick={() => setError('')} className="text-red-400 hover:text-red-600">✕</button>
        </div>
      )}

      <div className="flex gap-5 px-6 pb-8 items-start">
        {/* Left rail — grouped by the public ticket each task belongs to */}
        <aside className="w-56 shrink-0 bg-white rounded-xl border border-gray-100 p-2">
          <p className="text-[11px] uppercase tracking-wide text-gray-400 font-semibold px-2 py-1.5">Nézet</p>
          ...
          <p className="text-[11px] uppercase tracking-wide text-gray-400 font-semibold px-2 py-1.5 mt-2">
            Publikus feladataim
          </p>
          {linkable.length === 0 && (
            <p className="px-2 py-1.5 text-xs text-gray-400">Jelenleg nincs rád osztott nyitott feladat.</p>
          )}
          {linkable.map(t => (
            <RailButton key={t.id}
...
function RailButton({ active, onClick, label, count, muted }: {
  ...
  return (
    ...
    >
      <span className={`text-sm truncate ${muted && !active ? 'text-gray-400' : ''}`}>{label}</span>
